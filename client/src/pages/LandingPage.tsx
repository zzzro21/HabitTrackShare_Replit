  ...
  return (
    ...
    >
      ...
      <div className="relative flex flex-col items-center px-8 pt-6 pb-24 z-10 flex-grow">
        {/* 타원형 이미지 컨테이너 */}
        <div className={`w-full max-w-[480px] relative mt-8 mb-10 transition-all duration-500 ${showAnimation ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
          <div className="w-full overflow-hidden bg-orange-100 border-4 border-white shadow-xl" style={{ height: '560px', borderRadius: '50% / 65%' }}>
            <img
            ...
            {/* 메시지 버블 - 좌측 */}
            <div className={`absolute left-0 bottom-2/5 bg-white rounded-full shadow-lg flex items-center p-2.5 px-5 transform transition-all duration-500 ${showAnimation ? 'translate-x-0 opacity-100' : '-translate-x-20 opacity-0'}`}>
